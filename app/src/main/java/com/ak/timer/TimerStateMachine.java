...
public class TimerStateMachine implements ITimer,IPlaybackListener{
	...
	private class BaseState implements IState{
		...
		@Override
		public void exit() {
			Log.d(TAG, "exit called in " + mCurrentState.toString() + " exiting!!!");
			changeState(new StoppedState());

			if(mTimer != null)
				mTimer.cancel();
			if(mRestPeriodTimer != null)
				mRestPeriodTimer.cancel();

			if (mBuzzer != null) {
				if (mBuzzer.isPlaybackRunning()) {
					mBuzzer.stopSound();
				}
				mBuzzer = null;
			}
	...
	public boolean isTimerRunning() {
		...
		isTimerRunning = (mCurrentState instanceof StartedState
				|| (mCurrentState instanceof PlayMediaState)
				|| (mCurrentState instanceof RestPeriodState
				|| (mCurrentState instanceof PausedState)));
